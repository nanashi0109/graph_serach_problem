...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    dot_position, distance_to_dot = get_nearest_dot(position, food_list)
...
def get_nearest_dot(position, list_positions) -> Tuple[Tuple[int, int], int]:
    distances_to_dots = {}

    for food_position in list_positions:
        if position != food_position:
            distances_to_dots[get_distance(position, food_position)] = food_position

    if len(distances_to_dots.keys()) == 0:
        return ((0, 0), 0)
...
def get_distance_to_all_dots(position, dots_position):
    distance = 0
    for food_position in dots_position:
        if position != food_position:
            distance += get_distance(position, food_position)
    return distance
